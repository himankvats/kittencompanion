...
public class VetSummaryWebController {

    @Autowired
    private VetSummaryController vetSummaryController;

    @RequestMapping(value = {"/pets/{petId}/summaries", "/pets/{petId}/summaries/**"})
    public ResponseEntity<String> handle(HttpServletRequest req) throws IOException {
        ...
        APIGatewayProxyResponseEvent response = vetSummaryController.handleRequest(event, localContext());
    ...
    public ResponseEntity<String> health() {
        return ResponseEntity.ok("{\"status\":\"ok\",\"service\":\"vet-summary\"}");
    }
    ...
    private com.amazonaws.services.lambda.runtime.Context localContext() {
        return new com.amazonaws.services.lambda.runtime.Context() {
            ...
            public String getLogGroupName() { return "/local/vet-summary"; }
            public String getLogStreamName() { return "local"; }
            public String getFunctionName() { return "vet-summary-local"; }
            public String getFunctionVersion() { return "$LATEST"; }
            public String getInvokedFunctionArn() { return "arn:aws:lambda:local:0:function:vet-summary-local"; }
            public CognitoIdentity getIdentity() { return null; }
            public ClientContext getClientContext() { return null; }
            public int getRemainingTimeInMillis() { return 60000; }
